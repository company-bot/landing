...
export interface KidsHubData {
    title: string;
    subTitle: string;
    headerIcon: LucideIcon;
    emptyIcon: LucideIcon;
    color: 'cyan' | 'purple' | 'teal' | 'red' | 'blue' | 'yellow' | 'green';
    groups: {
        group1: AgeGroupData; // Ages 5-7
        group2: AgeGroupData; // Ages 7-10
        group3: AgeGroupData; // Ages 10-13
    };
}
...
const KidsHub: React.FC<KidsHubProps> = ({ data }) => {
    const { isDarkMode } = useStore();
    const [activeGroup, setActiveGroup] = useState<'group1' | 'group2' | 'group3' | null>(null);

    useEffect(() => {
        document.documentElement.classList.toggle('dark', isDarkMode);

        // Check URL params for initial group
        const params = new URLSearchParams(window.location.search);
        const levelParam = params.get('level');
        if (levelParam === 'group1' || levelParam === 'group2' || levelParam === 'group3') {
            setActiveGroup(levelParam);
        }
    }, [isDarkMode]);
        ...
        if (!activeGroup) {
            ...
            return (
                <div className="flex flex-col items-center justify-center py-20 text-gray-500 dark:text-gray-400 animate-fade-in-up">
                    <EmptyIcon className="w-20 h-20 mb-6 opacity-30 animate-pulse" />
                    <p className="text-xl font-display uppercase tracking-widest text-center">Select your age group to start the mission!</p>
                </div>
            );
        }
        ...
        return (
            <div className="animate-fade-in-up max-w-4xl mx-auto">
                <div className="mb-12 text-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-lg leading-relaxed max-w-2xl mx-auto">{groupData.description}</p>
                </div>
                    ...
                    <div className="relative mt-8 p-1 rounded-2xl bg-gradient-to-br from-cyan/20 to-purple/20 dark:from-cyan/10 dark:to-purple/10">
                        <div className="relative rounded-2xl glass-panel border border-cyan/30 dark:border-cyan/20 p-10 text-center overflow-hidden">
                            ...
                            <div className="relative z-10 flex flex-col items-center">
                                ...
                                <h3 className="text-2xl font-display font-bold text-gray-900 dark:text-white mb-3 uppercase tracking-wide">Continue The Adventure!</h3>
                                <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-lg text-lg">
                                    Unlock <span className="text-cyan font-bold">{lockedCount} more missions</span> to become a Master Coder!
                                </p>
                                <a
                                    href="/contact.html"
            ...
            <div className="relative z-10 flex flex-col min-h-screen">
                ...
                <main className="flex-grow pt-32 pb-20 px-4 max-w-7xl mx-auto w-full">
                    ...
                    <div className="flex justify-center mb-16">
                        <div className="flex p-1 glass-panel rounded-xl border border-cyan/30 dark:border-cyan/20 flex-wrap justify-center gap-2 sm:gap-0">
                            {[
                                { id: 'group1', label: 'Ages 5-7' },
                                { id: 'group2', label: 'Ages 7-10' },
                                { id: 'group3', label: 'Ages 10-13' }
                            ].map((group) => (
                                <button
                                    key={group.id}
                                    onClick={() => setActiveGroup(group.id as 'group1' | 'group2' | 'group3')}
                                    className={`px-6 sm:px-8 py-3 rounded-lg font-display font-bold text-sm uppercase tracking-widest transition-all duration-300 ${activeGroup === group.id
